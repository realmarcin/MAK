...
geneids_75 <- row.names(expr_data_75)
write.table(geneids_75, "yeast_cmonkey_75_geneids.txt", sep="\t")


###E coli
setwd("~/Documents/integr8_genom/Miner/rda/common/ecoli_data/")
data <- read.table("./Ecoli.txt",sep="\t",header=F,skip=1,row.names=1)
data <- data[,-1]
source("~/Documents/java/MAK/src/DataMining_R/Miner.R")
data_miss <- t(apply(data, 1, missfxn))
data_scale <- t(scale(t(data_miss )))