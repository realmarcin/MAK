...
public class MakeNullTest extends Program {
    public final static String version = "0.1";
    boolean debug = false;

    String[] valid_args = {
            "-source", "-intxt", "-infeat", "-inTF", "-inR", "-out", "-gmin", "-gmax",
    ...
    REXP Rexpr;
    org.rosuda.JRI.Rengine Rengine;
    ...
    */
     ...
     * @param args
     */
    public MakeNullTest(String[] args) {
    ...
    private void doUnchunked() {
        ...
        //save sample random_sizes numbers to a file -- currently generates a file which is too large for the FS
                /*int ecount = expmin;
                int gcount = genemin;
    ...
    private void computeCritChunked(int i, int j, int gcount, int ecount) {
        if (debug)
            System.out.println("computeCrit");
            ...
            for (int i = expstart; i < expmax + 1; ) {
                ...
                for (int ns = 0; ns < num_samples; ns++) {
                    ...
                    if (CRIT_MASTER_IND != 166 && CRIT_MASTER_IND != 167 &&
                        ...
                        //System.out.print(".");
                           /* if (debug)
                            ...
                            String rcall = ComputeCrit.buildCritCallTotal(false,
                                    assignCrit.CRITmean[CRIT_IND] == 1, onlymean, criterion);
                            if (debug)
                                System.out.println("R: " + rcall);
                    ...
                    if (critsraw == null) {
                        ...
                        for (int z = 0; z < critsraw.length; z++)
                            critsraw[z] = rand.nextDouble();
                    }
    ...
    public static void main(String[] args) {
        if (args.length >= 18 && args.length <= 34) {
            MakeNullTest rm = new MakeNullTest(args);
        } else {
            System.out.println("MakeNull version " + version);
            System.out.println("syntax: java DataMining.util.MakeNullTest\n" +
                    "<-source Miner.R>\n" +
                    "<-intxt input expr data txt>\n" +
                    "<-infeat input feat data txt>\n" +
                    "<-inR input data R>\n" +
                    "<-out outfile>\n" +
                    "<-gmin gene min>\n" +
                    "<-gmax gene max>\n" +
                    ...
                    "<-abs OPTIONAL binary abs/noabs 1/0 default 1,1,1>\n" +
                    "<-chunked T/F>\n" +
                    "<-frxnsign T/F>\n" +
