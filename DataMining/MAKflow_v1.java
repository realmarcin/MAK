...
public class MAKflow_v1 {
    ...
    private void runFlow(double startLevel, double stopLevel) {
        ...
        if (setLevel == 2 && (stopLevel >= 2 || stopLevel == 0)) {
            ...
            for (int it = 1; it <= maxnulljobs; it++) {
                makenull_pbs += "java -Xmx2G DataMining.MakeNull  " +//-Xms2G
                        "-source " + R_path + " " +
                ...
                makenull_pbs += "-frxnsign " + frxnsign_param + " " + "-abs " + absvect +
                        " &> " + localpath + output_subdir3 + it + ".out\n";
            }
        ...
        if (setLevel == 3 && (stopLevel >= 3 || stopLevel == 0)) {
            ...
            System.out.println("This is a dummy level. It serves as a reminder to run the task file on a cluster. Please do this before proceeding to level 4 which requires the output of those tasks!");
        ...
        for (int counter = 0; counter <= iter; counter++) {
            ...
            if (setLevel == 8 && (stopLevel >= 8 || stopLevel == 0)) {
                ...
                for (int it = 0; it < num_start_points; it++) {
                    task_script += "java  -Xmx1G DataMining.RunMiner ";
                    task_script += "-param " + localpath + input + prm.OUTPREFIX + "_" + startfileprefix + "_" + it + "__" + criterion.toUpperCase() + "__AG_" + MINER_STATIC.RANDOM_SEEDS[counter] + "__parameters.txt " +
                            "&> " + localpath + subdir_1 + prm.OUTPREFIX + "_" + startfileprefix + "_" + it + "__" + criterion.toUpperCase() + "__AG_" + MINER_STATIC.RANDOM_SEEDS[counter] + "_out.txt\n";
                }
            ...
            if (setLevel == 9 && (stopLevel >= 9 || stopLevel == 0)) {
                ...
                System.out.println("This is a dummy level. It serves as a reminder to run the task file on a cluster. Please do this before proceeding to level 10 which requires the output of those tasks!");
            ...
            if (setLevel == 11 && (stopLevel >= 11 || stopLevel == 0)) {
                ...
                System.out.println("This is a dummy level. It serves as a reminder to run the task file on a cluster. Please do this before proceeding to level 12 which requires the output of those tasks!");
        ...
        if (setLevel == 14 && (stopLevel >= 14 || stopLevel == 0)) {
            ...
            String ssh = "cp " + localpath + "level7_iter1.1/" + basename + "_1/*_0_* " + localpath + scriptbox + "param_example.txt";
            ...
            /*
             NOTE: LMM runs on Head Node. WARNING: Requires 22 Gb Memory!!!
            */
            ...
            runCmd(listmergeMembers_script, scriptbox, sl_lmm);
        ...
        if (!refine) {
           ...
           /*  */
            ...
            if (setLevel == 16 && (stopLevel >= 16 || stopLevel == 0)) {
                ...
                for (int it = 0; it < num_str_pt_refine; it++) {
                    ...
                    task_script += "-param " + localpath + input + prm.OUTPREFIX + "_" + refinement_prefix + "_" + it + "__" + criterion.toUpperCase() + "__AG_" + MINER_STATIC.RANDOM_SEEDS[0] + "__parameters.txt " +
                            "&> " + localpath + subdir_1 + prm.OUTPREFIX + "_" + refinement_prefix + "_" + it + "__" + criterion.toUpperCase() + "__AG_" + MINER_STATIC.RANDOM_SEEDS[0] + "_out.txt\n";
                }
                System.out.println("RunMiner task file written!");
            ...
            if (setLevel == 17 && (stopLevel >= 17 || stopLevel == 0)) {
                ...
                System.out.println("This is a dummy level. It serves as a reminder to run the task file on a cluster. Please do this before proceeding to level 18 which requires the output of those tasks!");
         ...
         /* level20 ListMergeMembers final */
            if (setLevel == 20 && (stopLevel >= 20 || stopLevel == 0)) {
                ...
                String sl_lmm = scriptbox + "listmergemembers.sh";
                /*
                NOTE: Running LMM On Head Node: Warning Requires 22Gb Memory!!!
                 */
                String listmergeMembers_script = "#!/bin/bash\n";
                        ...
                        if (param_key.equalsIgnoreCase("R")) {
                            ...
                            if (param_key.equalsIgnoreCase("data")) {
                                ...
                                try {
                                    ...
                                    criterion = "";
                                    // TODO: Ask Marcin if expression criteria is always necessary
     ...
     * @throws IOException
     */
    private void run_ht_helper(String taskfile, String scriptbox, String walltime, String constraint, int ntasks, boolean twoCoresPerTask) throws IOException {
        String sl_header = "";
        sl_header += "#!/bin/bash\n" +
                "#SBATCH --partition=jbei\n" +
                "#SBATCH --account=jbei\n" +
                "#SBATCH --mem=2000MB\n" +
        ...
        String node_task = "";
        if (twoCoresPerTask) {
            node_task = sl_header + "ht_helper.sh -t " + taskfile + " -n2";
        } else {
            node_task = sl_header + "ht_helper.sh -t " + taskfile + " -n1";
        }
    ...
    private void checkSLURMJobIsCompleted(String slurm_id, String scriptbox) throws IOException {
        ...
        while (jobIsRunning || priorCheckReport) {
            if (System.currentTimeMillis() - lastTime >= period) {
                ...
                String run_squeue = "";
                run_squeue += "squeue | grep 'jbei' | awk '{print $1}' > " + squeue_file;
    ...
    public static void main(String[] args) {
        if (args.length > 1) {
            ...
            MAKflow_v1 arg = new MAKflow_v1(args);
        } else {
            System.out.println("syntax: java DataMining.MAKflow_v1\n" + arg_desc_str
            );
        }
    }
}